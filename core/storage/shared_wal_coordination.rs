...
/// Maximum block count representable by the 32-bit entry count.
...
impl MappedSharedWalCoordination {
    ...
    pub(crate) fn record_frame(&self, page_id: u64, frame_id: u64) {
        ...
        // Once a frame is missing, keep the index a complete prefix. Resuming
        // after a transient growth failure would create an interior hole and
        // make rollback unable to prove that the retained index is complete.
    ...
    pub(crate) fn rollback_frames(&self, max_frame: u64) {
        ...
        // Overflow loses only the suffix after the last indexed frame. A
        // rollback into the indexed prefix restores a complete shared index.
        // Keep the flag when the retained range still includes missing frames.
...
mod tests {
    ...
    fn failed_growth_preserves_published_prefix_and_can_retry() {
        for fail_resize in [true, false] {
            ...
            assert_eq!(mapped.try_find_frame(7, 0, 1, None).unwrap(), Some(1));
            mapped
    ...
    #[test]
    fn lazy_reopen_checks_existence_without_mapping_the_entire_index() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("lazy.tshm");
        {
            let mapped = create_mapping(&path);
            mapped.record_frame(7, 1);
            mapped.reserve_frames(65 * 4096).unwrap();
        }
        let mut reopened = create_mapping(&path);
        assert_eq!(reopened.frame_index_blocks.read().len(), 0);
        assert!(reopened.has_visible_frame(1).unwrap());
        assert_eq!(reopened.frame_index_blocks.read().len(), 1);
        reopened.record_frame(9, 2);
        reopened.frame_index_blocks.write().clear();
        let original = reopened.file.clone();
        reopened.file = Arc::new(GrowthFailureFile {
            inner: original.clone(),
            fail_resize: false,
        });
        assert!(reopened.try_find_frame(7, 0, 2, None).is_err());
        assert!(reopened.try_iter_latest_frames(0, 2).is_err());
        reopened.file = original;
        assert_eq!(reopened.try_find_frame(7, 0, 2, None).unwrap(), Some(1));
    }
    ...
    fn overflow_must_remain_a_missing_suffix_until_rollback() {
        ...
        mapped.record_frame(7, 1);
        // Model frame 2 being omitted because growing/mapping a block failed.
